...
function enableButtons () {
	...
	btn = document.getElementById("period_button");
	btn.onclick=function(){greetByPeriod();};
	
	// Update the button label now that the button is active
	btn.value="Click for period, I guess?";
	
	btn = document.getElementById("gender_button");
	btn.onclick=function(){writeGender();};
	
	// Update the button label now that the button is active
	btn.value="Greet by gender";
}
...
function greetByName () {
	...
	// On success, pass the response to sayHelloCallback()
	var request = gapi.client.helloworldendpoints.sayHelloByName({'name': name, 'period': period});
	request.execute(sayHelloCallback);
}
...
function writeGender(){
	...
	var gender = getRadioVal( document.getElementById('genderForm'), 'gender' );
	var request = gapi.client.helloworldendpoints.writeGender({'gender': gender});
	request.execute(sayHelloCallback);
}
